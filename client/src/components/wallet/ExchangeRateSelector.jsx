...
const ExchangeRateSelector = ({
  ...
  useEffect(() => {
    if (selectedRate) {
      if (activeInput === 'cash' && cashAmount) {
        ...
        const cash = calculateCash(parseInt(pointsAmount), selectedRate.rate);
  ...
  const handleCashChange = value => {
    ...
    if (value && selectedRate) {
      const points = calculatePoints(parseFloat(value), selectedRate.rate);
      setPointsAmount(points.toString());
    ...
    <div className="exchange-rate-selector">
      ...
      {selectedRate && (
        <div className="rate-info">
          ...
          <span className="rate-value">{selectedRate.description}</span>
        </div>
      )}
        ...
        {mode === 'buy' ? (
          <>
            <div className="input-group">
              <label htmlFor="cash">{selectedCurrency} Amount</label>
              <input
                id="cash"
                type="number"
                min="0"
            ...
            <div className="input-group">
              <label htmlFor="points">Points You'll Receive</label>
              <input
                id="points"
                type="number"
                min="0"
          ...
          <>
            ...
            <div className="input-group">
              <label htmlFor="cash">{selectedCurrency} You'll Receive</label>
              <input
                id="cash"
                type="number"
                min="0"
