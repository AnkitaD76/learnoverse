...
export const initializeSocket = (token, userId, userName) => {
  console.log('🔌 initializeSocket called with:', {
    token: token ? 'YES' : 'NO',
    userId,
    userName,
  });
  ...
  if (!userId || !userName) {
    console.error('❌ VALIDATION FAILED - Missing required socket params:', {
      userId,
      userName,
    });
    return null;
  ...
  if (token) {
    ...
    console.log('🔐 Auth params with token:', {
      userId,
      userName,
      hasToken: true,
    });
    ...
    console.log(
      '🔐 No token provided - will use cookies. userId:',
      userId,
      'userName:',
      userName
    );
